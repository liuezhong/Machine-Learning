...
最小指的是：一次改变最小数目的αi
 ...
 （一）偏差、方差均衡（Bias/variance tradeoff)
...
k重交叉验证的优缺点：
优点：将数据分成了k份，每次都只需要留出1/k的数据；
缺点：它需要大量的计算，为了验证模型需要训练k次。
2、特征选择
前向搜索和后向搜索都是一种启发式搜索，不能保证它们会找到最好的特征子集。实际上选择最好的的特征子集是一个NP难题。
封装特征选择算法和过滤特征选择算法的比较：
封装特征选择算法在计算量可以满足要求的情况下效果是不错的，但是对类似于文本分类问题，因为文本分类问题特征数量特别多，很容易达到50000个特征，封装特征选择算法的计算量将会特别大。而过滤特征选择算法的虽然一般误差不会太低，从而导致假设的工作效果不会太好，但是所需要的计算量很小。
3、贝叶斯统计和规范化
   规范化：一种防止过拟合的方法，这种方法会保留所有的参数。

                            怎么样学习机器学习算法
 （一）学习算法的调试诊断法
1、判断问题是高偏差还是高方差
   比较Test error和Training error的差值，大的话是高方差，小的话是高偏差。
2、判断问题是出在算法的收敛性上还是出在一开始对目标函数的选择上
   SVM比贝叶斯logistic回归的效果更好，但是logistic回归运算更简单快速。
   α(θ)是加权准确率函数，对所有正确的样本进行加权求和。（非垃圾邮件的权值要比垃圾邮件的权值高，因为相对于垃圾邮件你更希望正确的预测非垃圾邮件）
   SVM比贝叶斯logistic回归的效果好所以α(θsvm)>α(θBLR)
   比较目标函数J(θsvm)与J(θBLR)的大小
   case 1：α(θsvm)>α(θBLR)；J(θsvm)>J(θBLR)
            说明BLR算法还没有收敛，因为BLR算法的目标是使得J(θ)取最大值，而SVM求得的参数能使J(θ)更大这说明此时J(θ)并未达到最大值，所以这是算法的收敛的问题
   case 1：α(θsvm)>α(θBLR)；J(θsvm)<=J(θBLR)（同时都用BLR的J(θ)）
            说明在目标函数的最大化上BLR比SVM做得好，但对于加权准确率SVM却取得更好的结果，这意味着即使某些参数不能使目标函数J(θ)最大化也能取得更高的加权准确率，即意味着J(θ)最大化并不一定表示加权准确率最大化，则表示目标函数J(θ)可能并不是一个好的目标函数。
 （二）误差分析和销蚀分析
 误差分析过程：逐渐地用基准值代替每个组件的输出并观察准确率的变化即将现有的系统性能和完美的性能进行对比。
 销蚀分析：尝试解释当前系统性能和一些效果很差的底线性能之间的差异。
 （三）关于怎样求解机器学习问题的建议
 1、过早优化问题（premature(statistical)optimization）
 ...
 （二）PCA
       PCA的应用
       潜在语义索引（Latent Semantic Indexing(LSI)）：用于文本分类
       PCA的实现
       奇异值分析（singular value decomposition)
 （二）ICA 独立成分分析
        CDF（Cumulative Distribution Functions）累积分布函数
...
基本概念是称为回报函数的概念
Psa（状态转换分布）：当你在任何给定的状态下采取任何行动时每个转移状态的概率。
R（奖励函数）
MDP的目标是找到一个策略（告诉我们每一个状态采取什么行动）来最大化总收益的期望值。
零成本吸收状态（zero cost absorbing status):这是一个状态，当你进入这个状态后，将没有更多的回报、永远留在这个状态。